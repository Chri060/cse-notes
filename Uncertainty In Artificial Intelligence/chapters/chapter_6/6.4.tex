...
\begin{example}
    ...
    \end{figure}
\end{example}
\begin{definition}[\textit{Perfect map}]
...
\begin{algorithm}[H]
    \caption{Variable elimination}
        \begin{algorithmic}[1]
            ...
            \EndProcedure 
            \Statex 
            \Function{EliminateSingleVariable}{$F,i$}
...
\begin{example}
    ...
    \end{figure}
    We want to compute $\Pr(W)$. To do this, we start by transforming the Bayesian network into a factor graph, where we define the factors as follows:
    \begin{itemize}
        \item $f_1(C)=\Pr(C)$.
        \item $f_2(S,C)=\Pr(S\mid C)$.
        \item $f_3(R,C)=\Pr(R\mid C)$.
        \item $f_4(W,S,R)=\Pr(W\mid S,R)$.
    \end{itemize}
