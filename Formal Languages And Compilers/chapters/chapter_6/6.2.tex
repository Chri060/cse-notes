...
\begin{table}[H]
    ...
    2             & \makecell[l]{$A \rightarrow \frac{b}{w}BA_1\dots A_n$ \\ $n \geq 0$ \\ $w \in \Delta^{\ast}, b \in \Sigma$ \\ $A_i \in \left( C \cup V \right)$}           & \makecell[l]{if $cc=b \land top=A$ then write($w$) \\ pop \\ push $A_n \dots A_1$ \\ advance the reading head}            & \makecell[l]{char $b$ was next expected and has been read \\ emit the target string $w$ \\ push the prediction string $A_1 \dots A_n$}    \\ \hline
