...
Consider a linear program $\min\{c^Tx\mid Ax=b,x\geq 0\}$ and a feasible basis $B$. 
\begin{proposition}
    ...
    \caption{Simplex}
        \begin{algorithmic}[1]
            ...
            \While{optimal $=$ false and unbounded $=$ false}
                \If{$\overline{a}[i,j] \geq 0\quad \forall j$}
                    \State optimal $=$ true
                \Else
                    \State Select a non-basic variable $x_s$ with $\overline{a}[0,s] < 0$
                    \If{$\overline{a}[i,s] \geq 0 \quad \forall j$}
