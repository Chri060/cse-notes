...
\chapter{Probabilistic reasoning}
    ...
    \end{definition}
    Representing a joint distribution of $M$ (binary) variables:
    \begin{itemize}
    ...
    \begin{example}
        ...
        Using the sum rule we can compute the marginal distribution of culprits: cook ($80\%$), and culprit $20\%$. 
        And the marginal distribution of weapons: pistol: ($20\%$), knife: ($54\%$), and poker: ($26\%$). 
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.2\linewidth]{images/GM.png}
        \end{figure}
        If at a certain point we discover the weapon, we can use the Bayes' theorem to compute who is probably the culprit. 
    \end{example}

    \section{Density estimation and its complexity}
    To compute the probability for logic expression is: 
    ...
    \begin{itemize}
        \item Can sort the records by probability, and thus spot weird records. 
        \item Can do inference. 
        \item Can be used for Bayes classifiers. 
    \end{itemize}
