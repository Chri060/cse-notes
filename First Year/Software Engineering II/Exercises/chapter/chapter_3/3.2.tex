...
\item Place order, which has as input the kitchenId.
    ...
    The handling of the order, and in particular its creation, is outside the scope of the \textt{KitchenDesigner} application; this is represented in the diagram by the fact that there is an interaction with the factory. 
...
The component \textt{DataManager} handles the data of the system, which consists essentially of users and kitchens. 
It provides interface \textt{DataManagerI}, which includes all necessary functions to handle CRUD operations on data.
The only backend component that does not need to interact with \textt{DataManager} is \textt{NotificationManager}, because it relies on information provided by \texttt{OrderManager}.
