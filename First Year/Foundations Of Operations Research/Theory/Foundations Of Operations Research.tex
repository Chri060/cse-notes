...
\chapter{Network optimization models}
    ...
    \begin{definition}
        ...
        \[\min{f(\boldsymbol{x})}\]
        such that $\boldsymbol{x} \in X \subseteq \mathbb{R}^n \rightarrow \mathbb{R}$, where: 
        \begin{itemize}
            \item The objective function $f:X \rightarrow \mathbb{R}$ is linear. 
            \item The feasible region $X=\{x \in \mathbb{R}^n|g_i(\boldsymbol{x})r_i0 \land i \in (1,m)\}$ with $r_i \in \{=,\geq,\leq\}$ and 
            $g_i:\mathbb{R}^n \rightarrow \mathbb{R}$ linear functions, for $i=1,\dots,m$.
        \end{itemize}

        $\boldsymbol{x}^{*} \in \mathbb{R}^n$ is an \emph{optimal solution} of the LP(1) if $f(\boldsymbol{x}^{*}) \leq f(\boldsymbol{x})$, $\forall \boldsymbol{x} \in X$. 
    \end{definition}
    The general form of a linear programming problem is the following: 
    \[\min{z}=\boldsymbol{c}^T\boldsymbol{x}\]
    such that $\boldsymbol{Ax} \geq \boldsymbol{b}$ and $\boldsymbol{x} \geq \boldsymbol{0}$. In this notation we have that $\boldsymbol{x}$
    is the vector of the decision variables. Note that the inequality for the matrix $\boldsymbol{A}$ can be changed to an equality, and the 
    ...
    \begin{itemize}
        ...
        \item If $\boldsymbol{x}_{ij}$ is unrestricted in sign we have that $\begin{cases}
            \boldsymbol{x}_j=\boldsymbol{x}_j^{+}-\boldsymbol{x}_j^{-} \\
            \boldsymbol{x}_j=\boldsymbol{x}_j^{+},\boldsymbol{x}_j^{-} \geq 0
            \end{cases}$
            After substituting $\boldsymbol{x}_j$ with $\boldsymbol{x}_j^{+}-\boldsymbol{x}_j^{-}$, we delete $\boldsymbol{x}_j$ from the 
            problem. 
    ...
    \begin{definition}
        ...
        \[[y_1,y_2]=\{x \in \mathbb{R}^n|x=\alpha y_1+(1-\alpha)y_2 \land \alpha \in [0,1]\} \]
    \end{definition}
    ...
    \begin{example}[Property]
        A non-empty polyhedron $P=\{x \in \mathbb{R}^n|\boldsymbol{Ax}=\boldsymbol{b},\boldsymbol{x} \geq \boldsymbol{0}\}$ (in standard form)
        or $P=\{\boldsymbol{x} \in \mathbb{R}^n|\boldsymbol{Ax}=\boldsymbol{b},\boldsymbol{x} \geq \boldsymbol{0}\}$ (in canonical form) has a 
