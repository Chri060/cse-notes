...
\begin{example}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.25\linewidth]{images/D5.png}
    \end{figure}
    ...
    \caption{Dijkstra}
        \begin{algorithmic}[1]
            ...
            \While {$\left\lvert S \right\rvert \neq n$}
                ...
                \State $S = S \cup \{u\}$
                \For {$(u,v) \in \delta^{+}(u) \text{ such that } L_v> L_u + c_{uv} $}
                    \State $L_v = L_u+c_{uv}$
                    \State $\text{pred}_v = u$
                    \State $X = X \cup \{v\}$
                \EndFor
...
\begin{definition}[\textit{Triangular operation}]
...
\end{definition}
\begin{definition}[\textit{Precedence constraint}]
