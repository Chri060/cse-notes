    ...
    \caption{Bayes filter algorithm}
        \begin{algorithmic}[1]
            \If{$d$ is a perceptual data item $z$} 
                \For{all $x$}
                    \State{$\text{Bel}^\prime(x)=\text{P}(z|x)\text{Bel}(x)$}
                \EndFor
                \State{normalize $\text{Bel}^\prime(x)$}
            \ElsIf{$d$ is an action data item $u$}
                \For{all $x$}
                    \State{$\text{Bel}^\prime(x)=\int\text{P}(x|u,x^\prime)\text{Bel}(x^\prime)\,dx^\prime$}
                \EndFor
            \EndIf
...
\begin{algorithm}[H]
    \caption{Discrete Bayes filter algorithm}
        \begin{algorithmic}[1]
            ...
            \ElsIf{$d$ is an action data item $u$}
                \For{all $x$}
                    \State{$\text{Bel}^\prime(x)=\sum_{x^\prime}\text{P}(x|u,x^\prime)\text{Bel}(x^\prime)$}
                \EndFor
            \EndIf
            \State \Return{$\text{Bel}^\prime(x)$}
        \end{algorithmic}
