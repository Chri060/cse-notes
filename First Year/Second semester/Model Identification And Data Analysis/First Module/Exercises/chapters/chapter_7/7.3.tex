...
\begin{enumerate}
    ...
    \item Derive with respect to the variable $\theta^{\ast}$: 
        \[\begin{cases}
            \dfrac{\partial \theta^{\ast}}{\partial a^\ast} = \dfrac{5}{2}a^\ast+1+ b^\ast\\
            \dfrac{\partial \theta^{\ast}}{\partial b^\ast} = \dfrac{5}{2} b^\ast+a^\ast
        \end{cases}\]
        We want a minimum, so we set those derivatives to zero:
        \[\begin{cases}
            \dfrac{5}{2}a^\ast+1+ b^\ast=0\\
            \dfrac{5}{2} b^\ast+a^\ast=0
