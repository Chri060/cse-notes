...
\chapter{Concurrency}
    ...
    \begin{itemize}
        ...
        \end{table}
        \begin{example}
            Given a table $X$ with eight tuples divided in two pages like this: 
            \begin{table}[H]
                \centering
                \begin{tabular}{cc}
    ...
    \begin{itemize}
        \item $r_{ts}(x)$ is always accepted. A copy $x_k$ is selected for reading such that:
            \begin{itemize}
                \item If $ts \geq WTMN(x)$, then $k=N$.
                \item Else take $k$ such that $WTMk(x) \leq ts < WTMk+1(x)$. 
            \end{itemize}
        \item $w_{ts}(x)$: 
            \begin{itemize}
    ...
    \begin{itemize}
        \item $r_{ts}(x)$ is always accepted. A copy $x_k$ is selected for reading such that:
            \begin{itemize}
                \item If $ts \geq WTMN(x)$, then $k=N$. 
                \item Else take $k$ such that $WTMk(x) \leq ts < WTM_{k+1}(x)$. 
            \end{itemize}
        \item $w_{ts}(x)$:
            \begin{itemize}
    ...
    \end{figure}
    The realization of TS-Multi gives the opportunity to introduce into the DBMS another isolation level called snapshot isolation. In this level only the $WTM$ is used. The rule 
