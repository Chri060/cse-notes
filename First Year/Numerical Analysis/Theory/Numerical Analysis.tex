...
\chapter{Nonlinear equations}
    ...
    \begin{itemize}
        \item On the error, stops if:
            \[\left\lvert x^{(k)}-\alpha \right\rvert \leq \epsilon_e\]
        \item On the residual, stops if:
            \[\left\lvert f\left(x^{(k)}\right) \right\rvert \leq \epsilon_r\]
        \item On the steps length, stops if:
            \[\left\lvert x^{(k)}-x^{(k-1)} \right\rvert \leq \epsilon_s\]
        \item On the max numbers of iterations, stops if:
            \[k \leq k_{max}\]
    \end{itemize}
    ...
    So, since the first part of the equation is similar to the condition on the error, we have that the stoppinc criterion will be: 
    ...
    \begin{table}[H]
        \centering
        \begin{tabular}{l|l}
        \textbf{Pros} & \begin{tabular}[c]{@{}l@{}}I can control the maximal error\\ Convergence is guaranteed\\ Use only evaluation of $f$\end{tabular} \\ \hline
        \textbf{Cons} & \begin{tabular}[c]{@{}l@{}}Work only if $f$ changes sign\\ Convergence is slow\end{tabular}                                     
        \end{tabular}
    \end{table}
